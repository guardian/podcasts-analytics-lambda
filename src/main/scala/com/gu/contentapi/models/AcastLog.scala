...
case class AcastLog(
  ...
  url: String,
  /**
   * (sc-status) One of the following values:
   *
   ...
   */
  status: String,
  /** (cs-referer) The name of the domain that originated the request. Common referrers include search engines, other websites that link directly to your objects, and your own website. */
  referrer: String,
  /** (cs-user-agent) The User Agent */
  userAgent: String,
  /** (cs-uri-query) The query string portion of the URI, if any. When a URI doesn't contain a query string, the value of cs-uri-query is a hyphen (-). */
  query: String,
  /**
  ...
  /**
   * (x-edge-response-result-type)How CloudFront classified the response just before returning the response to the viewer. See also x-edge-result-type in field 14.
   *
   * Possible values include:
   *
